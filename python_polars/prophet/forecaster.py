...
class Prophet(object):
    ...
    daily_seasonality: Fit daily seasonality.
        Can be 'auto', True, False, or a number of Fourier terms to generate.
    holidays: pl.DataFrame with columns holiday (string) and ds (date type)
    ...
    def __init__(
        ...
        if self.changepoints is not None:
            self.changepoints = pl.Series('ds', self.changepoints).cast(pl.Datetime)
    ...
    def validate_inputs(self):
        ...
        if self.holidays is not None:
            if not (
                isinstance(self.holidays, pl.DataFrame)
                and 'ds' in self.holidays.columns
                and 'holiday' in self.holidays.columns
            ):
                raise ValueError('holidays must be a DataFrame with "ds" and '
                                 '"holiday" columns.')
            self.holidays = self.holidays.with_column(pl.col('ds').cast(pl.Datetime))
            if (
                self.holidays.select(pl.col('ds').is_null()).to_series().any()
                or self.holidays.select(pl.col('holiday').is_null()).to_series().any()
            ):
                raise ValueError('Found a NaN in holidays dataframe.')
            has_lower = 'lower_window' in self.holidays.columns
            has_upper = 'upper_window' in self.holidays.columns
            if has_lower + has_upper == 1:
                raise ValueError('Holidays must have both lower_window and ' +
                                 'upper_window, or neither')
            if has_lower:
                if self.holidays.select(pl.col('lower_window').max()).item() > 0:
                    raise ValueError('Holiday lower_window should be <= 0')
                if self.holidays.select(pl.col('upper_window').min()).item() < 0:
    ...
    def setup_dataframe(self, df, initialize_scales=False):
        ...
        Parameters
        ----------
        df: pl.DataFrame with columns ds, y, and cap if logistic growth. Any
            specified additional regressors must also be present.
        initialize_scales: Boolean set scaling factors in self from df.

        Returns
        -------
        pl.DataFrame prepared for fitting or predicting.
        """
        if 'y' in df.columns:
            df = df.with_column(pl.col('y').cast(pl.Float64))
            if df.select(pl.col('y').is_infinite()).to_series().any():
                raise ValueError('Found infinity in column y.')
        if df['ds'].dtype == pl.Int64:
            df = df.with_column(pl.col('ds').cast(pl.Utf8))
        df = df.with_column(pl.col('ds').str.strptime(pl.Datetime))
        if df['ds'].dtype != pl.Datetime:
            raise ValueError('Failed to parse dates in column ds.')

        if df.select(pl.col('ds').is_null()).to_series().any():
        ...
        for name in self.extra_regressors:
            ...
            df = df.with_column(pl.col(name).cast(pl.Float64))
            if df.select(pl.col(name).is_null()).to_series().any():
                raise ValueError(
        ...
        for props in self.seasonalities.values():
            ...
            if condition_name is not None:
                ...
                if not df.select(pl.col(condition_name).is_in([True, False])).to_series().all():
                    raise ValueError(
                        'Found non-boolean in column {condition_name!r}'
                        .format(condition_name=condition_name)
                    )
                df = df.with_column(pl.col(condition_name).cast(pl.Boolean))
        ...
        if self.logistic_floor:
            ...
            if self.scaling == "absmax":
                df = df.with_column(pl.lit(0.0).alias('floor'))
            elif self.scaling == "minmax":
                df = df.with_column(pl.lit(self.y_min).alias('floor'))
        if self.growth == 'logistic':
            ...
            if (df.select(pl.col('cap') <= pl.col('floor')).to_series().any()):
                raise ValueError(
                    'cap must be greater than floor (which defaults to 0).'
                )
            df = df.with_column(((pl.col('cap') - pl.col('floor')) / self.y_scale).alias('cap_scaled'))

        df = df.with_column(((pl.col('ds').cast(pl.Date).cast(pl.Int64) - self.start.timestamp()) / self.t_scale.total_seconds()).alias('t'))
        if 'y' in df.columns:
            df = df.with_column(((pl.col('y') - pl.col('floor')) / self.y_scale).alias('y_scaled'))

        for name, props in self.extra_regressors.items():
            df = df.with_column(((pl.col(name) - props['mu']) / props['std']).alias(name))
        return df

    # Remaining methods need to be modified similarly to use polars instead of pandas
    ...
    def make_all_seasonality_features(self, df):
        ...
        Parameters
        ----------
        df: pl.DataFrame with dates for computing seasonality features and any
            added regressors.

        Returns
        -------
        pl.DataFrame with regression features.
        ...
        for name, props in self.seasonalities.items():
            ...
            if props['condition_name'] is not None:
                condition = df[props['condition_name']]
                features = features.with_columns([
                    pl.when(condition).then(pl.col(c)).otherwise(0).alias(c)
                    for c in features.columns
                ])
            seasonal_features.append(features)
            prior_scales.extend(
                [props['prior_scale']] * len(features.columns))
        ...
        for name, props in self.extra_regressors.items():
            seasonal_features.append(df.select([name]))
        ...
        if len(seasonal_features) == 0:
            seasonal_features.append(
                pl.DataFrame({'zeros': pl.lit(0).repeat(len(df))}))
            prior_scales.append(1.)

        seasonal_features = pl.concat(seasonal_features, how='horizontal')
    ...
    def regressor_column_matrix(self, seasonal_features, modes):
        ...
        component_cols: A binary indicator dataframe with columns seasonal
            ...
            that column is used in that component.
        modes: Updated input with combination components.
        """
        components = pl.DataFrame({
            'col': np.arange(len(seasonal_features.columns)),
            'component': [
        ...
        components = components.with_column(pl.lit(1).alias('value'))
        component_cols = components.pivot(
            values='value',
            index='col',
            columns='component',
        ...
        for name in ['additive_terms', 'multiplicative_terms']:
            if name not in component_cols.columns:
                component_cols = component_cols.with_column(
                    pl.lit(0).alias(name)
                )
        ...
        # Validation
        additive_terms = component_cols['additive_terms']
        multiplicative_terms = component_cols['multiplicative_terms']
        max_value = (additive_terms + multiplicative_terms).max()
        if max_value > 1:
        ...
        if self.train_component_cols is not None:
            ...
            if not component_cols.frame_equal(self.train_component_cols):
    ...
    def add_group_component(self, components, name, group):
        ...
        """
        new_comp = components.filter(pl.col('component').is_in(group)).clone()
        group_cols = new_comp['col'].unique()
        if len(group_cols) > 0:
            new_comp = pl.DataFrame({
                'col': group_cols,
                'component': [name] * len(group_cols)
            })
            components = pl.concat([components, new_comp])
        return components
    ...
    def set_auto_seasonalities(self):
        ...
        """
        first = self.history.select(pl.col('ds').min()).get_column('ds')[0]
        last = self.history.select(pl.col('ds').max()).get_column('ds')[0]
        dt = self.history['ds'].diff().drop_nulls()
        min_dt = dt.min()
    ...
    def linear_growth_init(df):
        ...
        Parameters
        ----------
        df: pl.DataFrame with columns ds (date), y_scaled (scaled time series),
            and t (scaled time).
    ...
    def logistic_growth_init(df):
        ...
        Parameters
        ----------
        df: pl.DataFrame with columns ds (date), cap_scaled (scaled capacity),
    ...
    def preprocess(self, df: pl.DataFrame, **kwargs) -> ModelInputData:
        """
        ...
        history = df.filter(pl.col('y').is_not_null()).clone()
        ...
        if self.growth in ['linear', 'flat']:
            ...
            cap = self.history['cap_scaled'].to_numpy()
        ...
        return ModelInputData(
            ...
            y=self.history['y_scaled'].to_numpy(),
            t=self.history['t'].to_numpy(),
    ...
    def fit(self, df: pl.DataFrame, **kwargs):
        ...
        Parameters
        ----------
        df: pl.DataFrame containing the history. Must have columns ds (date
    ...
    def predict(self, df: pl.DataFrame = None, vectorized: bool = True) -> pl.DataFrame:
        """Predict using the prophet model.

        Parameters
        ----------
        df: pl.DataFrame with dates for predictions (column ds), and capacity
        ...
        Returns
        -------
        A pl.DataFrame with the forecast components.
        """
        if self.history is None:
            raise Exception('Model has not been fit.')

        if df is None:
            df = self.history.clone()
        else:
            if df.shape[0] == 0:
                raise ValueError('Dataframe has no rows.')
            df = self.setup_dataframe(df.clone())

        df = df.with_column(
            pl.Series('trend', self.predict_trend(df))
        )
        ...
        df2 = df2.with_column(
            (
                pl.col('trend') * (1 + pl.col('multiplicative_terms')) +
                pl.col('additive_terms')
            ).alias('yhat')
        )
        return df2
    ...
    def predict_trend(self, df):
        ...
        return trend * self.y_scale + df['floor'].to_numpy()
    ...
    def predict_uncertainty(self, df: pl.DataFrame, vectorized: bool) -> pl.DataFrame:
    ...
    def sample_posterior_predictive(self, df: pl.DataFrame, vectorized: bool) -> Dict[str, np.ndarray]:
    ...
    def sample_model(self, df, seasonal_features, iteration, s_a, s_m) -> Dict[str, np.ndarray]:
        ...
        ----------
        df: Prediction dataframe.
        seasonal_features: pl.DataFrame of seasonal features.
        ...
        Xb_a = np.matmul(seasonal_features.to_numpy(),
                         beta * s_a.to_numpy()) * self.y_scale
        Xb_m = np.matmul(seasonal_features.to_numpy(), beta * s_m.to_numpy())
    ...
    def sample_model_vectorized(
        self,
        df: pl.DataFrame,
        seasonal_features: pl.DataFrame,
        iteration: int,
        s_a: np.ndarray,
        s_m: np.ndarray,
        n_samples: int,
    ...
    def sample_predictive_trend_vectorized(self, df: pl.DataFrame, n_samples: int, iteration: int = 0) -> np.ndarray:
        ...
        if self.growth == "linear":
            ...
            expected = self.piecewise_logistic(
                df["t"].to_numpy(), df["cap_scaled"].to_numpy(), deltas, k, m, self.changepoints_t
            )
        elif self.growth == "flat":
            expected = self.flat_trend(df["t"].to_numpy(), m)
        else:
    ...
    def _sample_uncertainty(self, df: pl.DataFrame, n_samples: int, iteration: int = 0) -> np.ndarray:
        ...
        if t_values.max() <= 1:
            ...
            future_df = df.filter(pl.col("t") > 1)
            n_length = future_df.shape[0]
            # handle 1 length futures by using history
            if n_length > 1:
                single_diff = np.diff(future_df["t"].to_numpy()).mean()
            else:
                single_diff = np.diff(self.history["t"].to_numpy()).mean()
            ...
            if t_values.min() <= 1:
                past_uncertainty = np.zeros((n_samples, np.sum(t_values <= 1)))
    ...
    def predictive_samples(self, df: pl.DataFrame, vectorized: bool = True):
    ...
    def make_future_dataframe(self, periods, freq='D', include_history=True):
        ...
        Returns
        -------
        pl.DataFrame that extends forward from the end of self.history for the
        ...
        if freq is None:
            ...
            freq = pd.infer_freq(self.history_dates[-5:])
        ...
        if include_history:
            dates = np.concatenate((self.history_dates.to_numpy(), dates))

        return pl.DataFrame({'ds': dates})
    ...
    def plot(self, fcst, ax=None, uncertainty=True, plot_cap=True,
        ...
        Parameters
        ----------
        fcst: pl.DataFrame output of self.predict.
